        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-6">
            {filteredCities.map(city => (
                <div key={city.id} className="p-6 bg-white rounded-xl shadow-sm flex flex-col hover:shadow-md transition-shadow">
                    <h4 className="text-gray-900 font-semibold text-lg">{city.label}</h4>
                    ...
                    <div className="flex flex-row justify-between gap-4 mb-2">
                        <p className="text-gray-700 text-sm">População:</p>
                    ...
                    <div className="flex flex-row justify-between gap-4 mb-4">
                        <p className="text-gray-700 text-sm">Renda média mensal:</p>
                        <p className="text-gray-800 font-semibold text-sm">
                            {formatCurrency(incomes?.[city.id])}
                        </p>
                    </div>

                    <Link href={`/estado/${stateId}/cidade/${city.id}`} className="mt-4 p-2 text-sm bg-primary-500 text-white hover:bg-primary-700 font-semibold rounded-md transition-colors text-center">
                        Ver Detalhes
                    </Link>
                </div>
            ))
